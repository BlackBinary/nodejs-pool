...
let etnAPIClient = request.createClient('https://etn.to/api/v1/etn2btc/');
...
let shapeshiftQueue = async.queue(function (task, callback) {
    ...
    async.waterfall([
        ...
        function (marketInfo, shapeshiftTxnData, body, intCallback) {
            ...
            global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, etn_amt, transaction_hash, mixin, fees, payees, exchange_rate, exchange_name, exchange_txn_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
let etnToQueue = async.queue(function (task, callback) {
    // http://etnto-api.readthedocs.io/en/latest/introduction.html
    ...
    async.waterfall([
        function (intCallback) {
            ...
            etnAPIClient.get('order_parameter_query/', function (err, res, body) {
                if (err) {
                    ...
                    if (body.lower_limit >= amtOfBTC) {
                        ...
                        return intCallback("Too much xmr to pay out to etn.to");
                    } else {
        ...
        function (btcValue, intCallback) {
            ...
            etnAPIClient.post('order_create/', {
                btc_amount: btcValue,
        ...
        function (txnID, intCallback) {
            // This function only exists because etn.to is a pretty little fucking princess.
            async.doUntil(function (etnCallback) {
                    etnAPIClient.post('order_status_query/', {uuid: txnID}, function (err, res, body) {
                        if (err) {
                            return intCallback(err);
                        } else if (body.error_msg) {
                            return intCallback(body.error_msg);
                        } else {
                            etnCallback(null, body.state);
                        }
                    });
                },
                function (etnCallback) {
                    return etnCallback !== "TO_BE_CREATED";
                },
                function () {
        ...
        function (txnID, intCallback) {
            etnAPIClient.post('order_status_query/', {uuid: txnID}, function (err, res, body) {
                if (err) {
                    ...
                    global.mysql.query("INSERT INTO etntoTxn (id, address, paymentID, depositType, withdrawl, withdrawlType, returnAddress, returnAddressType, txnStatus, amountDeposited, amountSent) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                        [txnID, body.etn_receiving_address, body.etn_required_payment_id, 'xmr', body.btc_dest_address, 'BTC', global.config.pool.address, 'xmr', body.state_str, global.support.decimalToCoin(body.etn_amount_total), global.support.decimalToCoin(body.btc_amount)]).then(function () {
                        return intCallback(null, body, global.support.decimalToCoin(body.etn_amount_total));
                    }).catch(function (error) {
        ...
        function (orderStatus, etnDeposit, intCallback) {
            // Make the payment to ShapeShift
            let paymentDetails = {
                destinations: [
                    {
                        amount: etnDeposit,
                        address: orderStatus.etn_receiving_address
                    }
                ],
                priority: global.config.payout.priority,
                mixin: global.config.payout.mixIn,
                payment_id: orderStatus.etn_required_payment_id
            };
            debug("Payment Details: " + JSON.stringify(paymentDetails));
            paymentQueue.push(paymentDetails, function (body) {
                if (body.fee && body.fee > 10) {
        ...
        function (orderStatus, body, intCallback) {
            ...
            // Need to add transaction.
            global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, etn_amt, transaction_hash, mixin, fees, payees, exchange_rate, exchange_name, exchange_txn_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [1, address, null, global.support.decimalToCoin(orderStatus.etn_amount_total), body.tx_hash.match(hexChars)[0], global.config.payout.mixIn, body.fee, 1, global.support.decimalToCoin(orderStatus.etn_price_btc), 'etnto', orderStatus.uuid]).then(function (result) {
            ...
            shapeshift.status(row.paymentID, function (err, status, returnData) {
                ...
                global.mysql.query("UPDATE shapeshiftTxn SET txnStatus = ? WHERE id = ?", [status, row.id]).then(function () {
                    if (status === 'complete') {
                        ...
                        global.mysql.query("SELECT id, etn_amt, address FROM transactions WHERE exchange_txn_id = ?", [row.id]).then(function (rows) {
                            ...
                            global.mysql.query("UPDATE balance SET amount = amount+? WHERE payment_address = ? limit 1", [rows[0].etn_amt, rows[0].address]);
                        });
...
function updatexmrToCompletion() {
    global.mysql.query("SELECT * FROM etntoTxn WHERE txnStatus NOT IN ('PAID', 'TIMED_OUT', 'NOT_FOUND', 'BTC_SENT')").then(function (rows) {
        rows.forEach(function (row) {
            etnAPIClient.post('order_status_query/', {uuid: row.id}, function (err, res, body) {
                if (err) {
                ...
                global.mysql.query("UPDATE etntoTxn SET txnStatus = ? WHERE id = ?", [body.state, row.id]).then(function () {
                    if (body.status === 'BTC_SENT') {
                        global.mysql.query("UPDATE etntoTxn SET transactionHash = ? WHERE id = ?", [body.btc_transaction_id, row.id]).then(function () {
                        ...
                        global.mysql.query("DELETE FROM etntoTxn WHERE id = ?", [row.id]);
                        global.mysql.query("SELECT id, etn_amt, address FROM transactions WHERE exchange_txn_id = ?", [row.id]).then(function (rows) {
                            global.mysql.query("DELETE FROM transactions WHERE id = ?", [rows[0].id]);
                            global.mysql.query("DELETE payments WHERE transaction_id = ?", [rows[0].id]);
                            global.mysql.query("UPDATE balance SET amount = amount+? WHERE payment_address = ? limit 1", [rows[0].etn_amt, rows[0].address]);
...
function determineBestExchange() {
    ...
    ], function (err, ssValue, etnToValue) {
        if (err) {
            return console.error("Error processing exchange value: " + JSON.stringify(err));
        }
        debug("ShapeShift Value: " + global.support.bitcoinCoinToDecimal(ssValue) + " xmr.to Value: " + global.support.bitcoinCoinToDecimal(etnToValue));
        if (ssValue >= etnToValue) {
            ...
            console.log("xmr.to is the better BTC exchange, current rate: " + global.support.bitcoinCoinToDecimal(etnToValue));
            bestExchange = 'etnto';
            global.mysql.query("UPDATE config SET item_value = 'etnto' where item='bestExchange'");
            global.mysql.query("UPDATE config SET item_value = ? where item='exchangeRate'", [etnToValue]);
        }
    });
}
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                ...
                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, etn_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    this.makePaymentAsIntegrated = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                debug("Successful payment sent to: " + identifier);
                global.mysql.query("INSERT INTO transactions (bitcoin, address, etn_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    this.makeBitcoinPayment = function () {
        ...
        let payee = this;
        if (bestExchange === 'etnto') {
            etnToQueue.push(functionalData, function (err, transactionID) {
                if (err) {
